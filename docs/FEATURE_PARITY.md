...
| Ventura native controls | Partial | Native window chrome is restored explicitly and the AppKit rendering bridge now synchronizes appearance, caret/text/control tint, including Add Knowledge Collection. Completed assistant turns regain actions and statistics. Contract tests pass; only Rosy Ventura can promote the visual repair. |
