...
| Credits and Router | Partial | Signed hosted inference, balance, Checkout, usage, activity, and metadata-only diagnostics already compile on Intel. Gate C1 adds an explicit Router opt-out and bounded top-up parsing. Credit units, redemption, usage center, and Premium Search remain Gates C2-C3 and must not be claimed from upstream commit coverage alone. |
