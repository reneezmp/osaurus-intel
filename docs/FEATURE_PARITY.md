...
| Agent Settings — Automation | Partial | Rosy retest on 2026-09-13 confirms schedule creation, persistence, and reachable next-run banner updates. Schedule and watcher cards do not expose their source-declared menus, leaving edit/pause/resume/run/delete unreachable. Monitoring-mode options remain unreadable. Schedule and watcher sessions lose their assigned folder; a watcher can create the session and user turn without ever producing an assistant response. |
| Agent Settings — Memory | Partial | Rosy retest on 2026-09-13 confirms Recent Chats, agent-aware New Chat, and the complete explanatory Database boundary. Pinned facts, episode summaries, and the compact empty state remain blocked from acceptance because `osaurus/qwen-3-8-max` distillation produces malformed-response errors and, after relaunch, `no_model:configured_unservable` skips. |
...
| Orchestrator | Partial | Rosy retest on 2026-09-13 confirms Settings routing, persistence, reset/inheritance, and chat use of editable model/prompt/generation values. Promotion failed: the Orchestrator chat receives no orchestration/delegation tools, so Gate 4 admission, permission, bounds, cancellation, and result contracts are unreachable there, and the built-in-only `orchestrator_config` claim is not user-observable. Intel also omits upstream's green built-in avatar and compiled-out built-in Orchestrator instructions; completed Orchestrator messages lack footer actions/statistics. The manual Settings delegation sheet and automated runtime tests do not establish a model-callable Orchestrator. See `ORCHESTRATOR_INTEL_PLAN.md`. |
