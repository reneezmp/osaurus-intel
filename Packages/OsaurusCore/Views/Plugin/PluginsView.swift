...
struct PluginsView: View {
    ...
    var body: some View {
        Group {
            if pluginManager.loadedPlugins.isEmpty && pluginManager.incompatiblePlugins.isEmpty {
                emptyState
            } else {
                pluginList
            }
        ...
        .onAppear {
            Task { await pluginManager.loadAll() }
        }
    }
    ...
    private var pluginList: some View {
        List {
            if !compatiblePlugins.isEmpty {
                Section("Available Plugins (\(compatiblePlugins.count))") {
                    ForEach(compatiblePlugins) { plugin in
                        PluginRow(name: plugin.name, version: plugin.version, pluginId: plugin.pluginId)
                    }
                }
            }

            if !degradedPlugins.isEmpty {
                Section("Degraded (\(degradedPlugins.count))") {
                    ForEach(degradedPlugins, id: \.id) { plugin in
                        VStack(alignment: .leading, spacing: 4) {
                            PluginRow(name: plugin.name, version: plugin.version, pluginId: plugin.pluginId)
                            if pluginManager.isDegraded(pluginId: plugin.pluginId) {
                                Text("Some optional features unavailable")
                                    .font(.caption)
            ...
            if !pluginManager.incompatiblePlugins.isEmpty {
                Section("Apple Silicon Required (\(pluginManager.incompatiblePlugins.count))") {
                    ForEach(pluginManager.incompatiblePlugins) { plugin in
                        VStack(alignment: .leading, spacing: 4) {
                            HStack {
                            ...
                            Text("Requires: \(plugin.missingRequired.joined(separator: ", "))")
                                .font(.caption2)
    ...
    private var compatiblePlugins: [PluginManager.LoadedPluginInfo] {
        pluginManager.loadedPlugins.filter { !pluginManager.isDegraded(pluginId: $0.pluginId) }
    }

    private var degradedPlugins: [PluginManager.LoadedPluginInfo] {
        pluginManager.loadedPlugins.filter { pluginManager.isDegraded(pluginId: $0.pluginId) }
    }
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            ...
            Text("No Plugins Installed")
                .font(.title3)
                .foregroundStyle(.secondary)
            Text("Plugins appear here after installation. Compatible plugins load automatically; plugins requiring Apple Silicon capabilities are listed for transparency.")
                .font(.caption)
                .multilineTextAlignment(.center)
                .foregroundStyle(.tertiary)
                .padding(.horizontal, 40)
            Text("Plugins directory: ~/.osaurus/Tools/")
                .font(.caption2)
